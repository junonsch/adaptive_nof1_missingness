...
from adaptive_nof1.metrics import score_missing_df
...
@dataclass
class SeriesOfMissingSimulationsData:
    configuration: dict
    simulations: List[MissingSimulationData]
    ...
    @staticmethod
    def score_missing_data(
        list_of_series: List[SeriesOfMissingSimulationsData],
        metrics,
        renaming: dict[str, Callable] = {},
    ):
        scored_df = score_missing_df(
            [
                simulation
    ...
    def __getitem__(self, index) -> SeriesOfMissingSimulationsData | MissingSimulationData:
